...
<div id="comments" class="comments-area p-2 oak-hovering-shadow default-max-width <?php echo get_option( 'show_avatars' ) ? 'show-avatars' : ''; ?>">

	<?php
	if ( have_comments() ) :
		;
		$comments_title = '';
		if ( '1' === $oak_comment_count ) :
			$comments_title =  '1 Reply to "' . get_the_title() . '"';
		else :
			$comments_title = $oak_comment_count . ' Replies to "' . get_the_title() . '"';
		endif;
		?>
		<h3 class="comments-title h4-display"><?= $comments_title ?></h3><!-- .comments-title -->
		...
		<ol class="comment-list">
			...
			<?php wp_list_comments(array(
						...
						'short_ping'	=> true,
						'type'			=> 'comment',
						'callback'		=> 'oak_default_formatted_comment'	)
					); ?>
		</ol><!-- .comment-list -->
	...
	comment_form(
		array(
			...
			'class_submit' =>   'btn oak-btn oak-btn-filled mt-4 oak-btn-rounded btn-md waves-effect waves-light',
		)
	);
	?>
